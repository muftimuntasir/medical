...
class AdmissionPayment(models.Model):
    ...
    def button_add_payment_action(self):
        ...
        eve_mee_obj = self.env['admission.payment.line']
        ...
        service_id = eve_mee_obj.create(service_dict)

        self.env.cr.execute("update leih_admission set due=%s, paid=%s where id=%s",
        ...
        period_id = self.env['account.period'].search([], limit=1).id
